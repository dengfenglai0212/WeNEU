 ...
 * 为了兼容修改的登录逻辑
 * 这里对登录的 SDK 进行重构
 * 公告：https://developers.weixin.qq.com/blogdetail?action=get_post_info&lang=zh_CN&token=&docid=0000a26e1aca6012e896a517556c01
 */
var constants = require('./constants');
var Session = require('./session');

/**
 * 登录，获取 code 和 encryptData
 */
    ...
    wx.login({
        success (loginResult) {
            wx.getUserInfo({
                ...
                fail (userError) {
                    cb(new Error('获取用户信息失败，请检查网络状态'), null)
                }
            });
        },
        fail (loginError) {
            cb(new Error('登录失败，请检查网络状态'), null)
        }
    })
}
 ...
 * @method
 * 进行服务器登录，以获得登录会话
 * 受限于的限制，本函数需要在 <button open-type="getUserInfo" bindgetuserinfo="bindGetUserInfo"></button> 的回调函数中调用
 ...
 * @param {Function} [opts.success] 可选。登录成功后的回调函数，参数 userInfo 用户信息
