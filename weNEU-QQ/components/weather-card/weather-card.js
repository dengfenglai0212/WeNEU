...
Component({
  ...
  methods: {
    ...
    getWeather: function() {
      var that = this;
      wx.request({
        url: 'https://www.zhongbr.cn:5000/weather?city=沈阳',
        method: 'GET',
        dataType: 'json',
        success: function(res) {
          ...
          } else if (that.data.weather.forecast[0].type.indexOf('霾') != -1) {
            that.setData({
              weatherClass: 'smog'
            });
          }
    ...
    setWeatherPicPos() {
      ...
      wx.getSystemInfo({
        success: e => {       
          $this.setData({
            weatherPicTop: e.screenHeight > 1000 ? "76rpx  !important" : $this.data.weatherPicTop
          });
        }
      })
    }
